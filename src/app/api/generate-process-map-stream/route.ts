...
import { parseTranscriptForUseCaseStream } from "@/lib/grok";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!useCase || !useCase.id || !useCase.title) {
      return new Response(
        JSON.stringify({ success: false, error: "Use case selection is required" }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          const generator = parseTranscriptForUseCaseStream(
            transcript,
            useCase as UseCase,
            apiKey,
            modelConfig
    ...
    console.error("Generate process map stream error:", error);
