...
import { extractUseCasesFromTranscript } from "@/lib/grok";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const data = await extractUseCasesFromTranscript(transcript, apiKey, modelConfig);

    return NextResponse.json({ success: true, data });
  } catch (error) {
    console.error("Extract use cases error:", error);
    const message =
      error instanceof Error ? error.message : "Failed to extract use cases";
    return NextResponse.json(
